...
const { GraphQLTokens: t } = require("./lexer")

class GraphQLParser extends Parser {
  // TODO: The logic for tracking the start/end positions of CST Nodes can easily consume 20-25% of the parsing time.
  // TODO: Attempt optimizations and / or evaluate other ways to acomplish the same thing
  cstPostTerminal(key, consumedToken) {
    if (consumedToken.isInsertedInRecovery !== true) {
      this.lastRealToken = consumedToken
      for (let i = this.CST_STACK.length - 1; i >= 0; i--) {
        let currCst = this.CST_STACK[i]
        if (currCst.children.loc === undefined) {
          currCst.children.loc = {
            start: consumedToken.startOffset,
            end: consumedToken.endOffset
            // TODO: Do we want to support the full grahql-js loc API
            // including the doubly linked list of Tokens or is this redundant because the CST
            // already contains this information?
          }
        } else {
          // if a CST already has a LOC it means that any previous CST must have
          // a LOC too because nested CST is always inside the "range" of its parent CST.
          break
        }
      }
    }
  ...
  cstFinallyStateUpdate() {
    const lastCstNode = this.CST_STACK[this.CST_STACK.length - 1]
    if (
      this.lastRealToken !== undefined &&
      // we encountered a real start Token
      lastCstNode.children.loc !== undefined
    ) {
      lastCstNode.children.loc.end = this.lastRealToken.endOffset
    }

    super.cstFinallyStateUpdate()
  }
  ...
  constructor(config) {
    ...
    $.RULE("OperationDefinition", () => {
      $.OR([
        {
          NAME: "$ImplicitQueryOperationDefinition",
          ALT: () => $.SUBRULE($.SelectionSet)
        },
        {
          NAME: "$ExplicitOperationDefinition",
          ALT: () => {
