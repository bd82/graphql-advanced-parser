...
class GraphQLParser extends Parser {
  ...
  cstPostTerminal(key, consumedToken) {
    if (consumedToken.isInsertedInRecovery !== true) {
      ...
      for (let i = this.CST_STACK.length - 1; i >= 0; i--) {
        ...
        if (currCst.loc === undefined) {
          currCst.loc = {
            start: consumedToken.startOffset,
            end: consumedToken.endOffset,
            // TODO: convert to graphQL Token
            startToken: consumedToken,
            endToken: consumedToken
          }
        } else {
  ...
  cstFinallyStateUpdate() {
    ...
    if (
      ...
      lastCstNode.loc !== undefined &&
      // END != START
      this.lastRealToken !== lastCstNode.loc.endToken
    ) {
      lastCstNode.loc.end = this.lastRealToken.endOffset
      lastCstNode.loc.endToken = this.lastRealToken
    }
