...
const apiText = fs.readFileSync(config.apiPath, "utf8")
const updatedVersionParserText = apiText.replace(
    `VERSION: "${config.currVersion}"`,
    `VERSION: "${newVersion}"`
)
fs.writeFileSync(config.apiPath, updatedVersionParserText)
